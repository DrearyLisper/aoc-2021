...
parseInput c = let l = lines c in (parsePixels $ head l, Image (-1, -1) (length (drop 2 l), length (l !! 2)) (parseImage $ drop 2 l) 0)
  where
...
part1 c = length $ filter (==1) $ pixels $ iterate step image !! 2
  where
    ...
    step (Image (x1, y1) (x2, y2) m dflt) = Image (x1-2, y1-2) (x2+2, y2+2) newM newDflt
      where
        newM = Map.fromList (map (\(point, pixels) -> (point, outputPixels !! bin2int pixels)) $ patches $ Image (x1, y1) (x2, y2) m dflt)
        newDflt = if dflt == 0 then outputPixels !! 0 else outputPixels !! bin2int [1, 1, 1, 1, 1, 1, 1, 1, 1]
